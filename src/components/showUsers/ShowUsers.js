...
class ShowUsers extends React.Component {
  ...
  pageChanged = async (pageNo) => {
    ...
    const url = `https://hostel-allotment-api.herokuapp.com/admin/${
      ...
      this.state.sortBy
    }:${this.state.order}${this.state.search ? "&" + this.state.search : ""}`;
  ...
  render() {
    return (
      <div className="ShowUsers">
        ...
        {this.state.totalUsers !== 0 && this.state.Users.length === 0 && (
          <p>No users Found!!</p>
        )}
        ...
        {this.state.totalUsers > this.state.limit && !this.state.search && (
          <Pagination
            totalPages={Math.ceil(this.state.totalUsers / this.state.limit)}
